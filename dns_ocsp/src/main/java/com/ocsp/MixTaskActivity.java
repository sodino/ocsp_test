...
public class MixTaskActivity extends AppCompatActivity implements Dns ,View.OnClickListener, Handler.Callback {
    ...
    private boolean runReq() {
        Request request = new Request.Builder()
                .url(isOcspHost ? Constant.URL_ocsp : Constant.URL_normal)
        ...
        try {
            ...
            if (response.code() == Constant.URL_RESP_CODE){
                ...
                //Log.d("com.ocsp.Test", "consume=" + consume);
    ...
    public List<InetAddress> lookup(String hostname) throws UnknownHostException {
        if (Constant.HOST_normal.equals(hostname)) {

            String ip = Constant.IP_normal;

            List<InetAddress> inetAddresses =
                    Arrays.asList(InetAddress.getAllByName(ip));
            return inetAddresses;
        } else if (Constant.HOST_ocsp.equals(hostname)) {

            String ip = Constant.IP_ocsp;
